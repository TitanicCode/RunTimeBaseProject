...
/**
 * 项目参数工具类
 * 额外需要：ContextHolderUtils User config.properties
 */
public class ResourceUtil {

	private static final ResourceBundle bundle = ResourceBundle.getBundle("config");

	public static final User getSessionUser(User user) {
		HttpSession session = ContextHolderUtils.getSession();
		User u = null;
		try {
			u = (User) session.getAttribute(user.getAccount()+"user");
		} catch (Exception e) {
			e.printStackTrace();
		}
		return u;
