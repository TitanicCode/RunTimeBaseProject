...
$(function() {
	...
	$("#export").off("click").on("click",function(){ //点击导出
		...
		if((paramsMap["ID_is_effect"] == null || paramsMap["ID_is_effect"]=='' )&& (paramsMap["phone_is_effect"] == null || paramsMap["phone_is_effect"]=='' )) {
			paramsMap["ID_is_effect"]="1";
			paramsMap["phone_is_effect"]="1";
		}
...
function initBindMethod(){
	...
	function showTable(obj) {
		...
		var grid = obj_arrays[obj_arrays.length] = Run.create("Grid", {
			id : "t1", //调用表格组件需要的ID
			url : ctx+"/financial/getInvalidData", //调用表格组件的JSON路径
