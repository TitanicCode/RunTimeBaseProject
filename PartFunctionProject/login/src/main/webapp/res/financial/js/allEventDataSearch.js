...
function initBindMethod(){
	...
	function showTable(obj) {
		...
		var grid = obj_arrays[obj_arrays.length] = Run.create("Grid", {
			id : "t1", //调用表格组件需要的ID
			url : ctx+"/financial/getAllEventDataByIdCardNo", //调用表格组件的JSON路径
			query : paramsMap, //调用表格组件传入的参数
//			query : {
//				"startTime":$("#startTime").val().trim(),
//				"endTime":$("#endTime").val().trim(),
//				"userName" : $("#userName").val().trim(),
//				"importFileName":$("#importFileName").val().trim()
//			}, //调用表格组件传入的参数
			...
			//colModel : colModelArray,//获取动态表中的所有字段mainDataTableName,phoneArea,idCardNo,phoneNum,huiyuanxingming
			colModel : [ {
				display : '事件名称',
				name : 'mainDataTableName'
			},  {
				display : '姓名',
				name : 'huiyuanxingming'
			},  {
				display : '手机号归属地',
				name : 'phoneArea'
			}, {
